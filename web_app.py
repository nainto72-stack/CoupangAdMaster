...
"""
    ...
    if svg_viewbox_match:
        ...
        native_width_px = width_pt * 1.333
        ...
        style_injection = f'<svg style="width: 100%; max-width: {int(displayed_width_px)}px; height: auto; display: block; margin: 0 auto;"'
        
        # Inject auto-aspect-ratio script to perfectly fit iframe height to screen width
        tooltip_html += f"""
<script>
    if (window.frameElement) {{
        window.frameElement.style.height = 'auto';
        window.frameElement.style.aspectRatio = "{width_pt} / {height_pt}";
    }}
</script>
"""
    else:
...
def render_large_trend_chart_streamlit(df, kw_data, memos):
    ...
    def setup_ax(ax):
        ax.set_facecolor('#0B0B1A')
        ax.tick_params(axis='x', labelcolor='#94A3B8', labelsize=fs_tick, rotation=45)
        ax.grid(True, axis='y', color='#1F2937', linestyle='--', alpha=0.4)
        for sp in ax.spines.values(): sp.set_color('#1F2937')
